...
class Model:
    ...
    def inference(self, input=None):
        ...
        images_re = tf.reshape(self.stimulus_placeholder, [-1, 6, 24, 24, 1])
        ...
        with tf.name_scope('Conv2'):
            ...
            W_conv2 = self.weight_variable([3, 5, 5, 32, 64])
        ...
        with tf.name_scope('fully_connected1'):
            ...
            h_pool2_flat = tf.reshape(h_pool2, [-1, 2 * 6 * 6 * 64])
            print_tensor_shape(h_pool2_flat, 'MaxPool2_flat shape')

            W_fc1 = self.weight_variable([2 * 6 * 6 * 64, 1024])
