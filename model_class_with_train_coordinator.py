...
def train():
    ...
    with tf.Graph().as_default():
        ...
        # Instantiate a model.
        model = Model(images, labels, keep_prob)
        ...
        train_writer = tf.summary.FileWriter(
            '/tmp/tensorflow/mnist/logs/sa_mnist_exp' + '/train', sess.graph)
        ...
        try:
            ...
            while not coord.should_stop():
                ...
                # Run one step of the model.  The return value is
                # the activations from the `train_op`.  To inspect the values
                # of your ops or variables, you may include them in
                # the list passed to sess.run() and the value tensors
                # will be returned in the tuple from the call.

                summary, _, loss = sess.run([merged, model.optimize, model.loss],
                                            {keep_prob: 0.5})
